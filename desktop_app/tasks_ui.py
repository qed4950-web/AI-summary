...
from core.tasks.store import Task, TaskStatus, TaskStore
...
class TaskManagerWindow(QDialog):
    ...
    def refresh_tasks(self):
        ...
        self._stop_loader(wait_ms=1500)
        loader = TaskLoader(self.store)
        self.loader = loader
        loader.tasks_loaded.connect(self._on_tasks_loaded)
    ...
    def _on_loader_finished(self):
        ...
        if loader is self.loader:
            self.loader = None
        if isinstance(loader, QThread):
            loader.deleteLater()

    def _stop_loader(self, *, wait_ms: int = 1500):
        loader = self.loader
        if loader is None:
            return
        if loader.isRunning():
            loader.requestInterruption()
            if not loader.wait(wait_ms):
                loader.terminate()
                loader.wait(500)
        self.loader = None

    def closeEvent(self, event):  # noqa: N802 - Qt naming convention
        self._stop_loader(wait_ms=2500)
